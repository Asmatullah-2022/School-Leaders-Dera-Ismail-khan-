import 'package:cloud_firestore/cloud_firestore.dart';

import '../../../core/firebase/firestore_converters.dart';
import '../../../core/firebase/firestore_paths.dart';
import '../../../core/firebase/scope_filter.dart';
import '../../../core/local_db/offline_write_helper.dart';
import '../domain/repositories/school_problem_repository.dart';
import 'models/school_problem_model.dart';

class SchoolProblemRepositoryImpl implements SchoolProblemRepository {
  SchoolProblemRepositoryImpl(this._firestore)
    : _writer = OfflineWriteHelper(
        firestore: _firestore,
        collectionName: FirestorePaths.schoolProblems,
        dateFields: SchoolProblemModel.dateFields,
      );

  final FirebaseFirestore _firestore;
  final OfflineWriteHelper _writer;

  CollectionReference<Map<String, dynamic>> get _collection =>
      _firestore.collection(FirestorePaths.schoolProblems);

  @override
  Stream<List<SchoolProblemModel>> watchByScope(ScopeFilter filter) {
    final Query<Map<String, dynamic>> query = filter
        .apply(_collection)
        .orderBy('dateReported', descending: true);
    return query.snapshots().map(
      (QuerySnapshot<Map<String, dynamic>> snap) => snap.docs
          .map((doc) => SchoolProblemModel.fromJson(firestoreDataToJson(doc.data())))
          .toList(),
    );
  }

  @override
  Future<SchoolProblemModel?> getById(String id) async {
    final DocumentSnapshot<Map<String, dynamic>> doc = await _collection.doc(id).get();
    if (!doc.exists || doc.data() == null) return null;
    return SchoolProblemModel.fromJson(firestoreDataToJson(doc.data()!));
  }

  @override
  Future<void> create(SchoolProblemModel problem) {
    return _writer.save(docId: problem.id, json: problem.toJson(), isCreate: true);
  }

  @override
  Future<void> update(SchoolProblemModel problem) {
    return _writer.save(docId: problem.id, json: problem.toJson(), isCreate: false);
  }

  @override
  Future<void> delete(String id) => _writer.delete(id);

  @override
  Future<void> queueEvidencePhoto(
    String problemId,
    String localFilePath, {
    bool isResolution = false,
  }) {
    return _writer.queuePhotoUpload(
      docId: problemId,
      filePath: localFilePath,
      storageField: isResolution ? 'resolutionEvidenceUrls' : 'evidencePhotoUrls',
    );
  }
}
